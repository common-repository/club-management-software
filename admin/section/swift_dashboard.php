 ...
 */
if (!function_exists('swiftbook_dashboard_callback')) {

    function swiftbook_dashboard_callback() {

        include_once( ABSPATH . WPINC . '/feed.php' );
        wp_enqueue_script('swift-form-jstz', "//swiftcloud.io/js/jstz.min.js", '', '', true);
        wp_enqueue_style('swift-dashboard', SWIFTBOOKSUBS__PLUGIN_URL . 'admin/css/swift-dashboard.css', '', '', '');
        wp_enqueue_script(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard-script', SWIFTBOOKSUBS__PLUGIN_URL . 'admin/js/swift-dashboard.js', array('jquery'), '', true);

        $subscribed = get_option(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe');
        $subs_checkbox_flag = isset($_COOKIE[SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_unsubscribe']) && $_COOKIE[SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_unsubscribe'] == 1 ? "block" : "none";
        ...
        <div class="wrap dashboard-wrap">
            ...
            <?php } ?>
            <!--subscribe box-->
            <div class="dashboard-subscribe-block" id="swiftbook_subscribe" style="display: <?php echo $subs_box_flag; ?>;">
                ...
                <form method="post" id="frm_swiftbook_dashboard_subscribe">
                    ...
                    <input type="hidden" name="formid" value="644" id="formid" />
                    ...
                    <div class="dashboard-col-8 col-left">
                        <div class="col-dashboard-block">
                            ...
                            <div class="col-dashboard-block-content">
                                <?php
                                $rss = fetch_feed('http://support.swiftcloud.io/tag/membership/feed');
                                $maxitems = 0;
                    ...
                    <div class="dashboard-col-4 col-right">
                        <div class="col-right-wrap">
                            ...
                            <div class="col-right-content">
                                <?php
                                $rss_recomm = fetch_feed('http://support.swiftcloud.io/tag/offers/feed');
                                $maxitems_recomm = 0;
 ...
 *      cookie value : 0 || 1
 */
add_action('wp_ajax_' . SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
add_action('wp_ajax_nopriv_' . SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
if (!function_exists(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback')) {

    function swiftbook_dashboard_subscribe_checkbox_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox') {
            setcookie(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
if (!function_exists(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback')) {

    function swiftbook_dashboard_close_subscribe_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_close_subscribe') {
            setcookie(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_unsubscribe', "1", time() + (10 * 365 * 24 * 60 * 60), "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_callback');

if (!function_exists(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe_callback')) {

    function swiftbook_dashboard_subscribe_callback() {
        check_ajax_referer('swiftdashboard_subs_form', 'swiftdashboard_subs_form');
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe') {
            parse_str($_POST['data'], $subscribe_form_data);

            update_option(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_subscribe', true);
            setcookie(SWIFTBOOKSUBS_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
